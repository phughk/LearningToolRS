...
mod tests {
  ...
  fn category_des_test() {
    ...
    let actual = Category::deserialize(&mut Deserializer::new(EventReader::new(input.as_bytes())));
  ...
  fn entry_des_test() {
    ...
    "#;
    let actual = LearningModuleEntry::deserialize(&mut Deserializer::new(EventReader::new(input.as_bytes())));
